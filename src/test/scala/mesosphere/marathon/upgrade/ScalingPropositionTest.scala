...
import mesosphere.marathon.state.{ PathId, Timestamp, InstanceHandling }
  ...
  "ScalingProposition.sortByConditionAndDate" when {
    "sorting a unreachable, unhealthy, running, staging and healthy tasks" should {
      ...
      "put unreachable before running" in {
        ScalingProposition.sortByConditionAndDate(InstanceHandling.DefaultKillSelection)(lostInstance, runningInstance) shouldBe true
        ScalingProposition.sortByConditionAndDate(InstanceHandling.DefaultKillSelection)(runningInstance, lostInstance) shouldBe false
      }
      "put unreachable before staging" in {
        ScalingProposition.sortByConditionAndDate(InstanceHandling.DefaultKillSelection)(lostInstance, stagingInstance) shouldBe true
        ScalingProposition.sortByConditionAndDate(InstanceHandling.DefaultKillSelection)(stagingInstance, lostInstance) shouldBe false
      }
      "put unreachable before starting" in {
        ScalingProposition.sortByConditionAndDate(InstanceHandling.DefaultKillSelection)(lostInstance, startingInstance) shouldBe true
        ScalingProposition.sortByConditionAndDate(InstanceHandling.DefaultKillSelection)(startingInstance, lostInstance) shouldBe false
      }
      "put staging before starting" in {
        ScalingProposition.sortByConditionAndDate(InstanceHandling.DefaultKillSelection)(startingInstance, stagingInstance) shouldBe false
        ScalingProposition.sortByConditionAndDate(InstanceHandling.DefaultKillSelection)(startingInstance, runningInstance) shouldBe true
      }
      "put staging before running" in {
        ScalingProposition.sortByConditionAndDate(InstanceHandling.DefaultKillSelection)(runningInstance, stagingInstance) shouldBe false
        ScalingProposition.sortByConditionAndDate(InstanceHandling.DefaultKillSelection)(stagingInstance, runningInstance) shouldBe true
      }
      "put starting before running" in {
        ScalingProposition.sortByConditionAndDate(InstanceHandling.DefaultKillSelection)(runningInstance, startingInstance) shouldBe false
        ScalingProposition.sortByConditionAndDate(InstanceHandling.DefaultKillSelection)(startingInstance, runningInstance) shouldBe true
      }
      "put younger staging before older staging" in {
        ScalingProposition.sortByConditionAndDate(InstanceHandling.DefaultKillSelection)(stagingInstanceOlder, stagingInstance) shouldBe false
        ScalingProposition.sortByConditionAndDate(InstanceHandling.DefaultKillSelection)(stagingInstance, stagingInstanceOlder) shouldBe true
      }
      "put younger starting before older starting" in {
        ScalingProposition.sortByConditionAndDate(InstanceHandling.DefaultKillSelection)(startingInstanceOlder, startingInstance) shouldBe false
        ScalingProposition.sortByConditionAndDate(InstanceHandling.DefaultKillSelection)(startingInstance, startingInstanceOlder) shouldBe true
      }
      "put younger running before older running " in {
        ScalingProposition.sortByConditionAndDate(InstanceHandling.DefaultKillSelection)(runningInstance, runningInstanceOlder) shouldBe true
        ScalingProposition.sortByConditionAndDate(InstanceHandling.DefaultKillSelection)(runningInstanceOlder, runningInstance) shouldBe false
      }
      "put younger staging before younger staging" in {
        ScalingProposition.sortByConditionAndDate(InstanceHandling.KillSelection.YoungestFirst)(stagingInstanceOlder, stagingInstance) shouldBe false
        ScalingProposition.sortByConditionAndDate(InstanceHandling.KillSelection.YoungestFirst)(stagingInstance, stagingInstanceOlder) shouldBe true
      }
      "put younger starting before younger starting" in {
        ScalingProposition.sortByConditionAndDate(InstanceHandling.KillSelection.YoungestFirst)(startingInstanceOlder, startingInstance) shouldBe false
        ScalingProposition.sortByConditionAndDate(InstanceHandling.KillSelection.YoungestFirst)(startingInstance, startingInstanceOlder) shouldBe true
      }
      "put younger running before younger running " in {
        ScalingProposition.sortByConditionAndDate(InstanceHandling.KillSelection.YoungestFirst)(runningInstance, runningInstanceOlder) shouldBe true
        ScalingProposition.sortByConditionAndDate(InstanceHandling.KillSelection.YoungestFirst)(runningInstanceOlder, runningInstance) shouldBe false
      }
    }
  }
