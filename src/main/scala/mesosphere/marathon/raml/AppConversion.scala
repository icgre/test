...
trait AppConversion extends ConstraintConversion with EnvVarConversion with SecretConversion
    with NetworkConversion with ReadinessConversions with HealthCheckConversion with InstanceHandlingConversion {
  ...
  implicit val appWriter: Writes[AppDefinition, App] = Writes { app =>
    App(
      ...
      instanceHandling = Some(app.instanceHandling.toRaml)
    )
  }
}
