...
import mesosphere.marathon.core.task.tracker.impl.{ ReservationSerializer, TaskSerializer }
...
class MigrationTo1_4_PersistenceStore(migration: Migration)(implicit
  ...
  private[migration] def marathonTaskToInstance(proto: MarathonTask): Instance = {
    ...
    val agentInfo: AgentInfo = {
      val host = if (proto.hasHost) {
        proto.getHost
      } else throw new IllegalArgumentException(s"task[${proto.getId}]: host must be set")

      val agentId = if (proto.hasSlaveId) {
        Some(proto.getSlaveId.getValue)
      } else Option.empty[String]

      val attributes = proto.getAttributesList.toIndexedSeq

      AgentInfo(host, agentId, attributes)
    }

    // TODO: use reservation to create the Instance
    val reservation: Option[Task.Reservation] = if (proto.hasReservation) {
